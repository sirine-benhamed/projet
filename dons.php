<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="home.css">
    <link rel="stylesheet" href="node_modules/bootstrap/dist/css/bootstrap.css">
    <link rel="stylesheet" href="dons.css">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>DS - Faire-un-don</title>
</head>

<body>
    <div class="ima">
        <div class="siss">
            <div class="sel">
                <div class="contenet">
                    <h1 class="title">JE FAIS UN DON</h1> </br></br></br>

                </div>
                <form action="test.php" method="post">
                    
                        <div class="container">




                        <div id="inp">
                            <div class="row">
                                <div class="col">
                                    <input type="text" class="form-control" name="carte"
                                        placeholder="Saisissez votre numéro de carte bancaire" required>
                                </div>
                                <div class="col">
                                    <input type="text" class="form-control" name="montant" placeholder="Montant libre" required>
                                </div>
                            </div>

                            <input  class="btn btn-warning" type="submit" value="Je crée" />
                            



                        </div>
                    </div>

                </form>
            </div>
        </div>
    </div>















    <div>

        <div class="fin">
            <div class="container">
                <div class="ff">
                    <div class="header__texturee">
                        <h4 class="coule">À PROPOS DE DonsSolidaires </h4>
                        <div class="div11">
                            <a href="nous.php" class="contac">Qui sommes nous</a><br><br>
                        </div>

                        <div class="div12">
                            <a href="connect.php" class="contac" target="_blank">Se connecter</a><br><br>
                        </div>
                        <div class="div13">
                            <a href="connect.php" class="contac" target="_blank">Faire un don</a>
                        </div>
                    </div>


                </div>
                <div class="ff" id="ff1">

                    <div class="header__texturee">
                        <h4 class="coule">CONDITIONS </h4>
                        <div class="div11">
                            <a href="confidentialite.php" class="contac" target="_blank">Politique de
                                confidentialité</a><br><br>
                            <a href="cookies.php" class="contac" target="_blank">Préférences de cookies</a><br><br>

                        </div>


                    </div>

                </div>


                <div class="ff" id="ff2">

                    <div class="header__texturee">
                        <h4 class="coule">NOS CAGNOTTES </h4>
                        <div class="div11">
                            <a href="naiss.php" class="contac">Cagnotte de naissance</a><br><br>
                        </div>


                    </div>

                </div>

            </div>




        </div>
    </div>

    </div>
    </div>




</body>

</html>